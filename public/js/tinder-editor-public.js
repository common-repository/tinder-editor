...
// tinder_data variables is generated dynamically via wp_localize_script
function ajaxRequest(item_id, item_type, item_value, target) {
	ajaxTarget = target;
	jQuery.post(tinder_data.ajax_url, { 
		_ajax_nonce: tinder_data.nonce,
		action: "tinder",
		item_id: item_id,
		item_type: item_type,
...
function catchIt(e)
{
	...
	if (!clicked.className.match(/tinder-editable/i)) {
		return;
	}
...
function attachTinder() {
	jQuery(".tinder-editable").click(
		function(e) {
			catchIt(e);
		}
	);
...
var tinderActive = false;

function toggleTinder() {
	...
	tinderActive = (tinderActive == true) ? false : true;
	if (tinderActive) {
		window.localStorage.setItem('tinderstate', 'editmode');
		jQuery('#wp-admin-bar-tinder_editor a').text('Preview Post');
		jQuery(".tinder").addClass("tinder-editable");
		attachTinder();
	}
	else {
		window.localStorage.setItem('tinderstate', 'previewmode');
		jQuery('#wp-admin-bar-tinder_editor a').text('Edit Post');
		jQuery(".tinder").removeClass("tinder-editable");
	}

	if (tinderActive) {
		jQuery(".tinder_content_preview").hide();
		jQuery(".tinder_content_edit").each(
			function() {
				jQuery(this).show();
				var iframe = jQuery('iframe',this).get(0);
				if (iframe)
					autoSize(iframe.id);
				jQuery(this).find('.mce-toolbar').last().addClass('tinder-row');
			}
		);
		...
		jQuery(".tinder_content_edit").hide();
		jQuery(".tinder_content_preview").show();
...
(function( $ ) {
	$( window ).load(function() {
		...
		jQuery("body").append('<div id="tinder_toggle" style="display:none"><a href="#" onclick="toggleTinder(); return false;" class="tinder-toolbar"><span class="tinder-icon"></span><span class="tinder-name">Tinder<sup>&reg;</sup></span></a></div>');
		...
		setTimeout(function() {
			if (!jQuery('#wpadminbar').length || !jQuery('#wpadminbar').is(":visible")) {
				jQuery('#tinder_toggle').show();

				$(window).scroll( function() {
					if ($(window).scrollTop() > $('body').offset().top)
						$('#tinder_toggle').addClass('floating');
					else
						$('#tinder_toggle').removeClass('floating');
				} );
			}
		}, 1000);

		if (window.localStorage.getItem('tinderstate') == 'editmode') {
			toggleTinder();
		}
